...
for i in range(population_size):
    nxm_random_num = list(
        np.random.permutation(num_job * num_mc))  # generate a random permutation of 0 to num_job*num_mc-1
...
print('pop', len(population_list[0]))
for n in range(num_iteration):
    ...
    for m in range(population_size):
        ...
        for i in range(num_job):
            if i in offspring_list[m]:
                print('m', m)
                print('i', i)
                print('offspring', offspring_list[m])
    ...
    for m in range(len(offspring_list)):
        ...
        if mutation_rate <= mutation_prob:
            m_chg = list(np.random.choice(num_job * num_mc, num_mutation_jobs,
    ...
    for m in range(population_size * 2):
        ...
        twet = sum(
            (1 / job_priority_duedate[j][0]) * d_record[j][0] + job_priority_duedate[j][0] * d_record[j][1] for j in
            j_keys)
        makespan = max(j_count.values())
        print("jcount value", j_count.values())
